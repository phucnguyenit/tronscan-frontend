...
export const reloadWallet = () => async (dispatch, getState) => {
  ...
  if (app.account.isLoggedIn) {
      let {balances, trc20token_balances, frozen, accountResource, delegated, tokenBalances, exchanges, ...wallet} = await Client.getAccountByAddress(app.account.address);
